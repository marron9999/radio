...
//			if(ys >= this.ym) ys = this.ym - 1;
	...
	i.message = function (wsx, msg) {
		...
		if(val[0] == "s") {
			...
			let x = 0, y = 0;
			while(true) {
				x  = parseInt(this.xm / 4) + parseInt(Math.random() * this.xm * 3 / 4);
				y  = parseInt(this.ym / 4) + parseInt(Math.random() * this.ym * 3 / 4);
				x = (x & 0x7ffe) + 1;
				y = (y & 0x7ffe) + 1;
				if(this.map1[y][x] == 0) break;
			}
			let z = 0;
			if(this.map1[y-1][x] == 1) {
				z = 9;
				if(this.map1[y][x-1] == 1) {
					z = 6;
					if(this.map1[y+1][x] == 1) {
						z = 3;
					}
...
const plugin = function(g) {
	...
	if(maze_i.map1.length == 0) {
		if(maze_i.size == undefined)
			maze_i.size = 1000;
		try {
			let json = fs.readFileSync("./maze/maze" + maze_i.size + ".json");
